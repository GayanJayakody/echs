...
class ProjectInProgress extends State{
    ...
    public function changestateoftheproject($project){
        ...
        $sql ='UPDATE Projects SET state='.$completed.' Where project_id="'.$project->projectid.'"';
        $conn->query($sql);
    }
}
...
class Project{
    ...
    public function donatetothisproject($amount){
        ...
        //if ((($this->raised)+($amount))<=$this->estimatedprojectcost){
            ...
            $sql ='UPDATE Projects SET raised='.$this->raised.' Where project_id="'.$this->projectid.'"';
            $conn->query($sql);
...
$sql = 'SELECT state,estimated_cost,raised FROM Projects Where project_id="'.$projectid.'"'; //reading things from the table
$result = $conn->query($sql);
if ($result->num_rows > 0){
    while($row = $result->fetch_assoc()){
        $completed=$row["state"];
        $estimatedprojectcost=$row["estimated_cost"];
        $raised=$row["raised"];
    }
}
